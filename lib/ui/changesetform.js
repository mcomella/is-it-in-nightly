...
var ChangesetBox = React.createClass({
    ...
    updateResult: function (err, isOnNightly, changesetIdForResult) {
        if (changesetIdForResult !== this.state.changesetIdForLastRequest) {
            ...
            if (err) {
                console.error('Error on ignored request: ' + err);
            }
            return;
        }
        ...
        if (err) {
            ...
            resultText = isOnNightly ? 'Yes!' : 'No';
        }
        this.setState({
            isLoading: false,
            isOnNightly: resultText,
